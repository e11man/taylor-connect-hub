...
// Organization password reset endpoint
app.post('/api/send-organization-password-reset', async (req, res) => {
  try {
    const { email } = req.body;
    ...
    console.log('Sending organization password reset code to:', email);

    // Call the Python script
    const pythonScriptPath = path.join(process.cwd(), 'email-service', 'send_organization_password_reset_email.py');
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('close', (code) => {
        if (code === 0) {
          ...
          console.log('Organization password reset email sent successfully via Python script');
          res.json({ 
            success: true, 
            message: 'Organization password reset code sent successfully',
            code: sentCode // For testing purposes
          });
          resolve();
        } else {
          console.error('Python script error:', errorOutput);
          res.status(500).json({ 
            error: 'Failed to send organization password reset email',
            details: errorOutput
          });
          reject();
        }
