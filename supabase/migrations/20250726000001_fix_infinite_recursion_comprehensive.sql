-- COMPREHENSIVE FIX: Eliminate infinite recursion by moving role logic to profiles table
-- This fix adds role to profiles and keeps it synchronized with user_roles table
...
-- 3. Create new is_admin function that queries profiles (no recursion possible)
...
-- 4. Create new is_pa function that queries profiles (no recursion possible)
...
SELECT 'Fix applied successfully! Profiles now have role column and functions use profiles table.' as status;