...
-- 1. Add new leadership roles to user_role enum if they don't exist
DO $$ BEGIN
...
-- 2. Add requested_role column to profiles table if it doesn't exist
ALTER TABLE public.profiles
ADD COLUMN IF NOT EXISTS requested_role public.user_role;

-- 3. Add role column to profiles table if it doesn't exist (for compatibility)
ALTER TABLE public.profiles
ADD COLUMN IF NOT EXISTS role public.user_role DEFAULT 'user';

-- 4. Update existing profiles to have proper role values
...
-- 5. Create or update the is_leadership function
...
-- 6. Create or update the is_admin function (non-recursive)
...
-- 7. Create or update the is_pa function (non-recursive)
...
-- 8. Force schema cache refresh by adding comments
